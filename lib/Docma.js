...
const jsdocx = require('neo-jsdoc-x');
...
class Docma {
    ...
    _parseJS(jsFiles, filePrefix) {
        ...
        function predicate(symbol) {
            ...
            // calculationRatio
            if (_files_key != symbol.meta.path){
                _files_key = symbol.meta.path;
                _files_index++;
                _self.$debug.log(`>> Speed ${parseFloat((_files_index / jsFiles.length) * 100).toFixed(2)} %, files path: ${_files_key}`);
            }
